    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="mb-1 text-2xl font-semibold tracking-tight text-slate-100">AI Brain</h1>
          ...
          >
            ...
            {s.count !== null && s.count > 0 && (
              <span className={`ml-1 rounded-full px-1.5 py-0.5 text-[10px] font-bold tabular-nums ${
                activeSection === s.id ? 'bg-orange-500/15 text-orange-300' : 'bg-white/10 text-slate-400'
              }`}>
                {s.count}
              </span>
            )}
      ...
      {activeSection === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-orange-500/30 bg-gradient-to-br from-orange-500/10 to-amber-500/10 p-6">
            <div className="mb-4 flex items-center gap-2 text-base font-semibold text-orange-300">
              <CalendarCheck size={16} /> Today's Game Plan
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              {todaysPlan.hotLeads.length > 0 && (
                <div className="rounded-lg border border-emerald-500/20 bg-apex-850 p-4">
                  ...
                  {todaysPlan.hotLeads.map(l => (
                    <div key={l.id} onClick={() => onLeadClick(l.id)} className="cursor-pointer rounded px-2 py-1.5 text-xs text-slate-300 hover:bg-white/5">
                      <span className="font-semibold text-slate-200">{l.co}</span> <span className="text-slate-400">· {l.pm || 'no contact'}</span>
                    </div>
                  ))}
                </div>
              )}
              {todaysPlan.callFirst.length > 0 && (
                <div className="rounded-lg border border-orange-500/20 bg-apex-850 p-4">
                  <div className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-widest text-orange-300">
                    <PhoneCall size={12} /> Call First — T1 with Contacts
                  </div>
                  {todaysPlan.callFirst.map(l => (
                    <div key={l.id} onClick={() => onLeadClick(l.id)} className="cursor-pointer rounded px-2 py-1.5 text-xs text-slate-300 hover:bg-white/5">
                      <span className="font-semibold text-slate-200">{l.co}</span> <span className="text-violet-300">· {l.pm}</span>
                    </div>
                  ))}
                </div>
              )}
              {todaysPlan.followUp.length > 0 && (
                <div className="rounded-lg border border-blue-500/20 bg-apex-850 p-4">
                  <div className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-widest text-blue-300">
                    <SendHorizonal size={12} /> Follow Up — In Pipeline
                  </div>
                  {todaysPlan.followUp.map(l => (
                    <div key={l.id} onClick={() => onLeadClick(l.id)} className="cursor-pointer rounded px-2 py-1.5 text-xs text-slate-300 hover:bg-white/5">
                      <span className="font-semibold text-slate-200">{l.co}</span> <span className="text-slate-400">· {l.status}</span>
                    </div>
                  ))}
                </div>
              )}
              {todaysPlan.needEmail.length > 0 && (
                <div className="rounded-lg border border-violet-500/20 bg-apex-850 p-4">
                  <div className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-widest text-violet-300">
                    <Mail size={12} /> Research — Need Emails
                  </div>
                  {todaysPlan.needEmail.map(l => (
                    <div key={l.id} onClick={() => onLeadClick(l.id)} className="cursor-pointer rounded px-2 py-1.5 text-xs text-slate-300 hover:bg-white/5">
                      <span className="font-semibold text-slate-200">{l.co}</span> <span className="text-slate-400">· has website, no email</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {[
              { label: 'Total Leads', value: stats.total, color: 'text-slate-100' },
              ...
              { label: 'Untouched', value: stats.untouched, color: 'text-slate-400' },
              { label: 'Dead', value: stats.dead, color: 'text-red-400' },
            ].map((s) => (
              <div key={s.label} className="rounded-xl bg-apex-850 ring-1 ring-white/10 p-4">
                <div className="text-[10px] font-medium uppercase tracking-widest text-slate-500">{s.label}</div>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-xl bg-apex-850 ring-1 ring-white/10 p-4">
              <div className="mb-2 text-xs font-semibold text-slate-100">Pipeline Health</div>
              <div className="flex gap-1 rounded-lg overflow-hidden h-3">
                ...
                {stats.untouched > 0 && <div style={{ width: `${(stats.untouched / stats.total) * 100}%` }} className="bg-slate-500/40" title="Untouched" />}
              ...
              <div className="mt-2 flex flex-wrap gap-3 text-[10px] text-slate-400">
                ...
                <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-slate-500/40" /> Untouched</span>
        ...
        <div className="space-y-4">
          {duplicates.length === 0 ? (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center">
              <div className="text-emerald-300 text-sm font-semibold">No duplicates found</div>
            ...
            <>
              ...
              {duplicates.map((group, idx) => (
                <div key={group[0]?.id || idx} id={`dup-group-${idx}`} className="rounded-xl bg-apex-850 ring-1 ring-white/10 p-4">
                  ...
                  <div className="space-y-2">
                    {group.map((lead) => (
                      <div key={lead.id} className="flex items-center justify-between rounded-lg bg-apex-800 ring-1 ring-white/10 p-3">
                        <div className="flex-1 cursor-pointer" onClick={() => onLeadClick(lead.id)}>
                          <div className="text-sm font-semibold text-slate-200">{lead.co}</div>
        ...
        <div className="space-y-2">
          ...
          {prioritized.map((item, idx) => (
            <div
              ...
              className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-apex-850 p-3 transition-colors hover:border-white/25"
            >
              <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-xs font-bold tabular-nums ${
                idx < 3 ? 'bg-orange-500/15 text-orange-300' :
                idx < 10 ? 'bg-white/10 text-slate-200' :
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-slate-200 truncate">{item.lead.co}</span>
                  {item.lead.t === 1 && <span className="rounded bg-orange-500/15 px-1.5 py-0.5 text-[9px] font-bold text-orange-300">T1</span>}
                  {item.lead.pm && <span className="rounded bg-violet-500/15 px-1.5 py-0.5 text-[9px] font-bold text-violet-300">PM</span>}
                </div>
                <div className="text-[11px] text-slate-400">
      ...
      {activeSection === 'stale' && (
        <div className="space-y-2">
          ...
          {staleLeads.length === 0 ? (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center">
              <div className="text-emerald-300 text-sm font-semibold">No stale leads</div>
            ...
            staleLeads.map((lead) => (
              <div key={lead.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-apex-850 p-3">
                <div className="flex-1 cursor-pointer" onClick={() => onLeadClick(lead.id)}>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-slate-200">{lead.co}</span>
                    <span className="rounded bg-white/10 px-1.5 py-0.5 text-[9px] font-bold text-slate-300">{lead.status}</span>
                  </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="rounded-lg border border-blue-500/20 bg-blue-500/10 px-3 py-1.5 text-[11px] font-medium text-blue-300 transition-colors hover:bg-blue-500/20"
                  >
                    Follow Up
                  </button>
                  <button
                    onClick={() => setStatus(lead.id, 'dead')}
                    className="rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-[11px] font-medium text-red-300 transition-colors hover:bg-red-500/20"
                  >
                    Mark Dead
                  </button>
                </div>
      ...
      {activeSection === 'enrich' && (
        <div className="space-y-3">
          ...
          {needsResearch.length === 0 ? (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center">
              <div className="text-emerald-300 text-sm font-semibold">All leads have been researched!</div>
            ...
            <>
              ...
              {needsResearch.map(({ lead, hasWeb, missing }) => (
                <div key={lead.id} className={`flex items-center justify-between rounded-xl border p-3 ${
                  hasWeb ? 'border-white/10 bg-apex-850' : 'border-white/10 bg-apex-850 opacity-50'
                }`}>
                  <div className="min-w-0 flex-1 cursor-pointer" onClick={() => onLeadClick(lead.id)}>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-sm font-semibold text-slate-200">{lead.co}</span>
                      {lead.t === 1 && <span className="rounded bg-orange-500/15 px-1.5 py-0.5 text-[9px] font-bold text-orange-300">T1</span>}
                      {!hasWeb && (
                        <span className="flex items-center gap-1 rounded bg-red-500/10 px-1.5 py-0.5 text-[9px] font-bold text-red-300">
                          No Website
                        </span>
                      )}
                      {hasWeb && (
                        <span className="flex max-w-[200px] items-center gap-1 rounded bg-emerald-500/10 px-1.5 py-0.5 text-[9px] font-bold text-emerald-300">
                    ...
                    <div className="mt-0.5 flex flex-wrap gap-1">
                      {missing.map(m => (
                        <span key={m} className="text-[10px] text-red-300/80">missing {m}</span>
                      ))}
                    </div>
                  </div>
                  <div className="flex gap-2 ml-3">
                    {hasWeb && handleAI ? (
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg border border-violet-500/20 bg-violet-500/10 px-3 py-1.5 text-[11px] font-medium text-violet-300 transition-colors hover:bg-violet-500/20 disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-apex-800 px-3 py-1.5 text-[11px] font-medium text-slate-400 transition-colors hover:bg-white/10 hover:text-slate-200"
                      >
          ...
          {missingInfo.slice(0, 30).map(({ lead, gaps }) => (
            <div key={lead.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-apex-850 p-3">
              ...
              <div className="flex flex-wrap gap-1">
                {gaps.map((g) => (
                  <span key={g} className="rounded bg-red-500/10 px-2 py-0.5 text-[10px] font-medium text-red-300">
                    {g}
                  </span>
                ))}
              </div>
